...
| HTML generator / JSON-LD / AI-naming comments / embedded data URIs | Strip tags and comments; clean embedded data URIs | `clean_file.py` | Loses meta; cleans embedded rasters; `content` prose under a non-naming `<meta name>` is left alone | Yes |
| Markdown AI frontmatter keys / AI-naming comments / embedded data URIs | Drop keys and comments outside code fences; clean embedded data URIs | `clean_file.py` | Loses YAML keys; cleans embedded rasters; prose values under a non-naming key are left alone | Yes |
