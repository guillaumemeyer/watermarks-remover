...
def test_docx_preserves_app_and_strips_customxml(tmp_path: Path):
    data = _make_docx_with_app()
    cleaned, actions = clean_docx(data)
    assert any("customXml" in a or "drop" in a for a in actions)
    with zipfile.ZipFile(io.BytesIO(cleaned)) as zf:
        ...
        # Application is preserved (library signature, not AI provenance)
        assert "Claude AI Writer" in app
...
def test_docx_metadata_vendor_word_in_app_is_not_flagged():
    from container_meta import inspect_docx

    data = _make_docx_with_app("Claude AI Writer")
    _has_c2pa, has_ai, findings, _ = inspect_docx(data)
    # customXml with c2pa contentcredentials IS flagged as AI metadata
    assert has_ai
    assert any("c2pa" in f.lower() or "contentcredentials" in f.lower() for f in findings)
    # But Application field (library signature) is NOT flagged
    assert not any("Application" in f and "Claude" in f for f in findings)
...
def test_docx_scrubs_docprops_provenance_fields_preserves_app_and_appversion():
    ...
    # Provenance fields in core.xml are emptied...
    for field in (
        "dc:creator",
        "cp:lastModifiedBy",
        "dc:description",
    ...
    # not AI provenance markers). Emptying them makes Word reject the file.
    assert "<Application>ChatGPT</Application>" in app
    assert "<AppVersion>16.0</AppVersion>" in app
    # Company and Manager are still scrubbed
    ...
    # Application and AppVersion should NOT appear in scrub actions
    assert not any("scrub docProps/app.xml field Application" in a for a in actions)
    assert not any("scrub docProps/app.xml field AppVersion" in a for a in actions)
